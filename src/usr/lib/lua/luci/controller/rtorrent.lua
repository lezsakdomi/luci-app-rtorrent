...
function index()
	...
	entry({"admin", "rtorrent", "main"}, form("rtorrent/main"), "Torrent List", 10).leaf = true
	entry({"admin", "rtorrent", "add"}, form("rtorrent/add", {autoapply=true}), "Add Torrent", 20)
	entry({"admin", "rtorrent", "rss"}, arcombine(cbi("rtorrent/rss"), cbi("rtorrent/rss-rule")), "RSS Downloader", 30).leaf = true
	entry({"admin", "rtorrent", "admin"}, form("rtorrent/admin/rtorrent"), "Admin", 40)
